...
const Orders = () => {
    ...
    useEffect(()=>{
        const getData = async()=>{
            ...
            try{
                if(!currentUser){
                    console.log("currentUser is not existed");
                    return
                };
            ...
            {loading ? (
                ...
                orders?.length > 0 ? (
                            ...
                            <div className="space-y-6 divide-y divide-gray-900/10">
                                {orders?.map((order: OrderTypes)=>{
                                    const totalAmt = order?.orderItems.reduce(
                                        (acc, item) => 
                                            acc + (item?.discountedPrice * item?.quantity || 0),0
                                    );
                                    return (
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <Link to={"/product"} className="mt-2 bg-gray-800 text-gray-100 px-6 py-2 rounded-md hover:bg-black hover:text-white duration-200">
                            Go to shopping
                        </Link>
                    </div>
                )

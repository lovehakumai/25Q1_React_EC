...
const FavoriteProduct = ({productId}:{productId: number}) => {
    const favoriteProduct = store((state)=>state.favoriteProduct);
    const removeFromFavorite = store((state)=>state.removeFromFavorite)
    const favoritItem = favoriteProduct.find((item)=>item._id === productId);
    const navigate = useNavigate();
    if(!favoritItem) {
        console.log("No favoriteItems");
        return
    };
    ...
    return (
        ...
        <div className="min-w-0 flex-1 lg:flex lg:flex-col">
          <div className="lg:flex-1">
            <div className="sm:flex">
              <div>
                <h4 className="font-medium text-gray-900">{favoritItem?.name}</h4>
                <p className="mt-2 hidden text-sm text-gray-500 sm:block">
                  {favoritItem?.description}
                </p>
                <p className="text-sm mt-1">
                  Brand: <span className="font-medium">{favoritItem?.brand}</span>
                </p>
                <p className="text-sm mt-1">
                  Category:{" "}
                  <span className="font-medium">{favoritItem?.category}</span>
                </p>
              </div>
              <span
                onClick={() => {
                  removeFromFavorite(favoritItem?._id);
            ...
            <div className="flex text-sm items-center gap-6 font-medium py-4">
              <AddToCartBtn product={favoritItem} className="w-32" />
            </div>
          </div>
          <p>
            You are saving{" "}
            <span className="text-sm font-semibold text-green-500">
              <FormattedPrice
                amount={favoritItem?.regularPrice - favoritItem?.discountedPrice}
              />
            </span>{" "}
            upon purchase
          </p>
        ...
        <div
          onClick={() => navigate(`/product/${favoritItem?._id}`)}
